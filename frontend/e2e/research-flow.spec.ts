...
// This spec exercises real Supabase Auth plus the FastAPI backend's research
// endpoints (fixture providers, so no external API keys are needed once
// Supabase auth works). It requires both a live Supabase project and the
// backend running at NEXT_PUBLIC_API_BASE_URL, so it's skipped unless
// E2E_SUPABASE_LIVE=1 — see specs/phase-3-research.md, Testing checklist,
// and specs/phase-1-foundation.md, Risks and Assumptions, for why this
// can't run in this environment.
const liveSupabaseConfigured = process.env.E2E_SUPABASE_LIVE === "1";

test.describe("campaign research run", () => {
  test.skip(
    !liveSupabaseConfigured,
    "Requires a live Supabase project and a running backend (set E2E_SUPABASE_LIVE=1)",
  );

  test("running a confirmed campaign produces leads with evidence and scores", async ({ page }) => {
